...
export async function ensureAccount(
  ...
  const { rows } = await db.query<Viewer>(
    `INSERT INTO users(id,identity_key,nickname,role) VALUES($1,$2,$3,$4) ON CONFLICT(identity_key) DO UPDATE SET role=EXCLUDED.role RETURNING id,nickname,role,auth_provider AS "authProvider"`,
...
export async function getViewer(db: Database, token?: string): Promise<Viewer | null> {
  ...
  const result = await db.query<Viewer>(
    `SELECT u.id,u.nickname,u.role,u.auth_provider AS "authProvider" FROM users u JOIN sessions s ON s.user_id=u.id WHERE s.token_hash=$1 AND s.expires_at>now() AND (u.auth_provider='uis' OR EXISTS(SELECT 1 FROM admin_credentials a WHERE a.user_id=u.id AND a.enabled))`,
    [tokenHash(token)],
  );
